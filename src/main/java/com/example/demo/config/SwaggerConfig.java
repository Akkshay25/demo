...
public class SwaggerConfig {
	...
	public Docket eDesignApi(SwaggerConfigProperties swaggerConfigProperties) {
		...
		doc.globalOperationParameters(
				Arrays.asList(new ParameterBuilder().name("Authorization").description("Authorization Token")
						.modelRef(new ModelRef("string")).parameterType("header").required(false).build()));
		return doc.apiInfo(apiInfo(swaggerConfigProperties))
